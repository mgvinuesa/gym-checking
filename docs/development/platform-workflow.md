...
Keep Vercel Authentication enabled for preview deployments and configure its
Protection Bypass for Automation:

1. In the Vercel project, open **Settings > Deployment Protection**.
2. Generate a Protection Bypass for Automation secret.
3. In the GitHub repository, open **Settings > Secrets and variables > Actions**.
4. Create the repository secret `VERCEL_AUTOMATION_BYPASS_SECRET` with the value
   generated by Vercel.

The deployment verification workflow sends this value only through the
`x-vercel-protection-bypass` request header. Never store it in a committed env
file, expose it with a `NEXT_PUBLIC_` prefix or print it in workflow logs.
